...
def visualize_genome(genome: Genome) -> None:
    ...
    layers = [
        map_layer_to_graph(layer, layer_idx, max_width)
        for layer_idx, layer in enumerate(un_mapped_layers)
    ]
...
def map_to_layers(genome: Genome) -> list[list[NodeGene]]:
    ...
    layers.append(
        genome.input_nodes
        + ([genome.bias_node] if genome.bias_node is not None else [])
    )
    ...
    while len(queue) > 0:
        ...
        for con in genome.get_node_connections_out(current):
            ...
            queue_lookup[con.get_destination_node()] = max(
                queue_lookup[con.get_destination_node()], lvl + 1
            )
...
def map_layer_to_graph(
    layer: list[NodeGene], layer_number: int, max_width: int
) -> list[GraphNode]:
...
def map_connections(
    connections: list[ConnectionGene], nodes: list[GraphNode]
) -> list[GraphEdge]:
    conns = []
    ...
    for con in connections:
        ...
        if con.get_destination_node().get_id() not in node_lookup_table:
            print(
                f"connection {con} was skipped because the destination does not exists"
            )
            continue
