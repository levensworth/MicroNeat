...
class NeuralNetwork:
    ...
    def process(self, inputs: list[float]) -> list[float]:
        ...
        if self._genome.bias_node is not None:
            self._activation_lookup[self._genome.bias_node.get_id()] = (
                self._genome.bias_node.activate(1)
            )

        for node in self._genome.output_nodes:
            try:
                result.append(self._apply_computation(node))
    ...
    def _apply_computation(self, current_node: "NodeGene") -> float:
        current_value = 0.0
        self._activation_lookup[current_node.get_id()] = (
            0  # this is a measure to stop looping forever
        )
        for con in self._genome.get_node_connections_in(current_node):
            ...
            if con.get_source_node().get_id() in self._activation_lookup:
                computation_value = self._activation_lookup[
                    con.get_source_node().get_id()
                ]
            else:
