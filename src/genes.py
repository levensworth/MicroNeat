...
class NodeGene:
    def __init__(
        self,
        node_id: int,
        node_type: "NodeGene.NodeTypeEnum",
        activation_func: typing.Callable[[float], float],
    ) -> None:
        self._id = node_id
...
class ConnectionGene:
    def __init__(
        self,
        connection_id: int,
        source_node: NodeGene,
        destination_node: NodeGene,
        weight: float,
        is_enabled: bool = True,
    ) -> None:
        assert (
            destination_node.get_type() != NodeGene.NodeTypeEnum.BIAS
        ), "you can not create a in connection to a bias node"
